 ...
 * @category Zircote
 * @package Zircote_Tool
 */
/**
 *
 *
 */
class Zircote_Tool_DatabaseSchemaProvider extends Zend_Tool_Project_Provider_Abstract
{
    /**
    ...
    public function updateTo($version, $env='development', $dir='./scripts/migrations')
    {
        ...
        try {
            ...
            $manager = new Zircote_Db_Schema_Manager($dir, $db, $this->getTablePrefix());

            $result = $manager->updateTo($version);

            switch ($result) {
                case Zircote_Db_Schema_Manager::RESULT_AT_CURRENT_VERSION:
                    if (!$version) {
                ...
                case Zircote_Db_Schema_Manager::RESULT_NO_MIGRATIONS_FOUND :
    ...
    protected function _init($env)
    {
        ...
        $autoloader->registerNamespace('Zircote_');
    }
    ...
    protected function _getDbAdapter()
    {
        if ((null === $this->_db)) {
            ...
            if(!$this->_db instanceof Zend_Db_Adapter_Abstract) {
                throw new Zircote_Db_Schema_Exception('Database was not initialized');
            }
        }
        return $this->_db;
    }
    ...
    public function createMigration($migrationName){
        ...
        $docblock = new Zend_CodeGenerator_Php_Docblock(array(
            'shortDescription' => 'Generated Migration class',
            'longDescription'  => 'This is a autogenerated migration class.',
            'tags'             => array(
                array(
                    'name'        => '@author',
                    'description' => ' ',
        ...
        $data .= $cg->setName($migrationName)
            ->setExtendedClass('Zircote_Db_Schema_AbstractChange')
            ->setMethods($methods)
            ->setDocblock($docblock)
            ->setProperties($properties)
            ->generate();
        file_put_contents($this->_getDirectory() . DIRECTORY_SEPARATOR . $fileName, $data);
